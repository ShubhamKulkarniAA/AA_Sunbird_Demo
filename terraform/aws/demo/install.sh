...
# Helper function to perform a deep cleanup of Terraform/Terragrunt artifacts
deep_clean_artifacts() {
  echo "--- Performing comprehensive cleanup of Terraform and Terragrunt artifacts ---"
  # Remove all .terragrunt-cache directories
  find "${BASE_DIR}/terraform" -type d -name ".terragrunt-cache" -exec rm -rf {} + || true
  # Remove all .terraform directories (where providers are stored)
  find "${BASE_DIR}/terraform" -type d -name ".terraform" -exec rm -rf {} + || true
  # Remove Terraform lock files
  find "${BASE_DIR}/terraform" -type f -name ".terraform.lock.hcl" -exec rm -f {} + || true
  echo "Cleanup complete."
}

# Helper function to explicitly set Terragrunt backend environment variables
set_terragrunt_backend_vars() {
  echo "--- Setting Terragrunt backend environment variables ---"
  export AWS_TERRAFORM_BACKEND_BUCKET="demotfstatesunbird"
  export AWS_TERRAFORM_BACKEND_DYNAMODB_TABLE="demo-terraform-lock"
  echo "Terragrunt backend variables set."
}

# Helper function to create Terraform state backend
create_tf_backend() {
...
# Helper function to create AWS resources with Terragrunt
create_tf_resources() {
  echo "--- Creating AWS Resources with Terragrunt ---"
  ...
  # The deep_clean_artifacts function now handles cache cleanup.
  ...
  # Updated to use run-all explicitly for init, and then apply --all
  (cd "$SCRIPT_DIR" && terragrunt init -migrate-state && terragrunt apply --all -auto-approve --terragrunt-non-interactive) || { echo "Error: Terragrunt apply failed."; exit 1; }


  echo "AWS resources created."

  echo "--- Fetching EKS Cluster Name and S3 Bucket Name from Terragrunt Outputs ---"
...
# Helper function to set up Kubernetes namespaces and configmaps
setup_kubernetes_prerequisites() {
  echo "--- Ensuring Kubernetes Namespaces and ConfigMaps ---"
...
# Helper function to configure certificate keys in Registry Service
certificate_config() {
...
# Helper function to extract learnbb sub-charts
extract_learnbb_subcharts() {
  echo "--- Extracting Learnbb Sub-Charts ---"
  ...
  (
    ...
    for chart_archive in *.tgz; do
        if [[ -f "$chart_archive" ]]; then
            ...
            mkdir -p "$dir_name"
            tar -xzf "$chart_archive" -C "$dir_name" --strip-components=1
            if [ $? -eq 0 ]; then
...
# Helper function to install/upgrade a single Helm component
install_helm_component() {
  ...
  echo "--- Installing/Upgrading component: $component ---"
  ...
  touch "$global_cloud_values_path"
  ...
  helm upgrade --install "$component" "$chart_path" --namespace sunbird \
    ...
    --timeout 20m --debug --wait --wait-for-jobs || { echo "Error: Helm installation failed for $component."; exit 1; }
...
# Helper function to install all Helm components
install_helm_components() {
    ...
    echo "--- Updating learnbb Helm Chart Dependencies ---"
...
# Helper function for NodeBB post-installation plugins
post_install_nodebb_plugins() {
  echo "--- Post-Install: NodeBB Plugin Activation & Rebuild ---"
...
# Helper function to verify DNS mapping
dns_mapping() {
...
# Helper function to check Kubernetes pod statuses
check_pod_status() {
  ...
  for component in "${components[@]}"; do
    ...
    case "$component" in
      ...
      "learnbb") label_selector="app.kubernetes.io/instance=learnbb" ;;
  ...
  echo "--- Checking Core Database/Infra Pods Status ---"
...
main() {
  ...
  check_tools
  deep_clean_artifacts # Added for comprehensive cleanup
  set_terragrunt_backend_vars # Added for explicit backend variables
  create_tf_backend
  backup_configs
  create_tf_resources
  ...
  echo "🎉 Sunbird EKS Platform Installation Complete! 🎉"
}
