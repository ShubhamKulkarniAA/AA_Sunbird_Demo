...
check_aws_credentials() {
    ...
    if [[ -z "${AWS_ACCESS_KEY_ID:-}" || -z "${AWS_SECRET_ACCESS_KEY:-}" || -z "${AWS_REGION:-}" ]]; then
        ...
        echo "Please export these environment variables before running the script (e.g., export AWS_ACCESS_KEY_ID=...)."
        exit 1
    fi
...
create_tf_resources() {
    ...
    # Using the directly exposed 'eks_cluster_name' output as confirmed by your output.tf
    ...
    # Ensure AWS CLI is installed for update-kubeconfig
...
certificate_keys() {
    ...
    local cert_dir="$SCRIPT_DIR" # Keys will be created next to install.sh
...
certificate_config() {
    ...
    # Use deployment name for exec, kubectl will find the running pod
    local nodebb_pod_selector="deploy/nodebb"
    echo "Updating apt and installing jq on NodeBB pod ($nodebb_pod_selector)..."
    kubectl -n sunbird exec "$nodebb_pod_selector" -- apt update -y || { echo "❌ Failed to update apt on NodeBB pod."; return 1; }
    kubectl -n sunbird exec "$nodebb_pod_selector" -- apt install -y jq || { echo "❌ Failed to install jq on NodeBB pod."; return 1; }

    echo "Checking for existing Certificate RSA public key in Registry Service..."
    local CERTKEY=$(kubectl -n sunbird exec "$nodebb_pod_selector" -- \
    ...
    if [[ -z "$CERTKEY" ]]; then
        ...
        local global_values_path="$SCRIPT_DIR/global-values.yaml" # Referencing path relative to script
        ...
        kubectl -n sunbird exec "$nodebb_pod_selector" -- curl --location --request POST 'http://registry-service:8081/api/v1/PublicKey' \
...
install_component() {
    ...
    # Reference global-values.yaml and global-cloud-values.yaml from SCRIPT_DIR
...
install_helm_components() {
    ...
    for component in "${components[@]}"; do
        # Call install_component from within the helmcharts base dir
...
post_install_nodebb_plugins() {
    ...
    # Get the actual pod name to be more robust, or use deploy/nodebb
...
dns_mapping() {
    ...
    while true; do
        ...
        echo "Waiting for domain name from lms-env configmap..."
        sleep 10
    done
    ...
    while true; do
        ...
        echo "Waiting for public IP from nginx-public-ingress service..."
        sleep 10
    done
    echo "✅ Found public IP: $public_ip"

    local dns_propagation_timeout=$((SECONDS + 1200)) # 20 minutes
...
main() {
    ...
    check_aws_credentials # This checks env vars and exits if not found

    local current_run_dir=$(pwd)
    if [[ $(basename "$current_run_dir") != "template" ]]; then
        echo "Navigating to 'template' directory: $SCRIPT_DIR"
        cd "$SCRIPT_DIR" || { echo "❌ Cannot navigate to $SCRIPT_DIR. Ensure script is run from or moved to the correct location."; exit 1; }
    ...
    if [[ -f tf.sh ]]; then
        ...
        echo "❌ tf.sh not found after backend creation. This is unexpected and crucial."
        exit 1
    fi
    ...
    # Now change to Helm charts base directory for Helm deployments
    echo "Navigating to Helm charts directory: $HELM_CHARTS_BASE_DIR"
    cd "$HELM_CHARTS_BASE_DIR" || { echo "❌ Cannot navigate to Helm charts directory: $HELM_CHARTS_BASE_DIR. Check path."; exit 1; }
