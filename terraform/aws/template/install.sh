...
check_aws_credentials() {
    ...
    if [[ -z "${AWS_ACCESS_KEY_ID:-}" || -z "${AWS_SECRET_ACCESS_KEY:-}" || -z "${AWS_REGION:-}" ]]; then
        echo "❌ AWS credentials (AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY) or AWS_REGION not found."
        echo "Please export these environment variables (e.g., export AWS_ACCESS_KEY_ID='AKIA...')."
        exit 1
    fi
...
clear_terragrunt_cache() {
    ...
    find . -maxdepth 1 -type d -name ".terragrunt-cache" -exec rm -rf {} + || true
...
create_tf_resources() {
    ...
    if [[ -z "$EKS_CLUSTER_NAME" || "$EKS_CLUSTER_NAME" == "null" ]]; then
        ...
        echo "Please ensure 'eks_cluster_name' is correctly exported and 'terragrunt apply' ran successfully."
        exit 1
    fi
    echo "Identified EKS Cluster Name: $EKS_CLUSTER_NAME"

    if ! command -v aws &>/dev/null; then
        echo "❌ Error: AWS CLI not found. Install AWS CLI to update kubeconfig."
        exit 1
    fi
...
certificate_config() {
    ...
    wait_for_app_ready "nodebb" "sunbird" "300" || { echo "❌ NodeBB not ready for certificate config. Aborting."; return 1; }
    ...
    if [[ -z "$CERTKEY" ]]; then
        ...
        local global_values_path="$SCRIPT_DIR/global-values.yaml"
...
dns_mapping() {
    ...
    echo "ACTION REQUIRED: Add/update your DNS A record for $domain_name to point to $public_ip."
...
main() {
    ...
    kubectl cluster-info || { echo "❌ kubectl cluster-info failed after provisioning."; exit 1; }
    kubectl get nodes || { echo "❌ kubectl get nodes failed after provisioning."; exit 1; }
